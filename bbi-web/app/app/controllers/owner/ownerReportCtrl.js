angular.module("bbManager").controller("ownerReportCtrl", [
	"$scope",
	"$location",
	"$auth",
	"NgMap",
	...
	function (
		...
		getOwnerReportCounts = function () {
			ManagementReportService.getOwnerReportCounts().then(function (
				result
			) {
  ...
  =====================*/
		...
		$scope.searchableAreas = function (query) {
			return ManagementReportService.searchAreas(
				query.toLowerCase()
			).then(function (res) {
				return res;
			});
			...
			if ($scope.areaObj == null) {
				...
				if (
					...
					for (var item in product.dates) {
						// product.dates[item].endDate = moment(
						// 	product.dates[item].endDate
						// ).format("YYYY-MM-DD");
						// product.dates[item].startDate = moment(
						// 	product.dates[item].startDate
						// ).format("YYYY-MM-DD");
						if($scope.areaObj.area_time_zone_type != null){
							const startDate = moment(product.dates[item].startDate);   
		...
		$scope.rqstHrdngsOpts = {
			...
			isInvalidDate: function (dt) {
				if ($scope.unavailalbeDateRanges) {
					for (
						var i = 0;
						i < $scope.unavailalbeDateRanges.length;
						i++
					) {
						if (
							moment(dt) >=
		...
		$scope.navigateToDetailPage = function (index) {
			//$location.path("/owner/demo---landmark-ooh/hoardling-list");
			$state.go("owner.hoarding-list");
		};
		//map ctr//
			...
			if (booked_from && booked_to) {
				...
				MapService.filterProducts(filterObj).then(function (markers) {
					...
					if (markers.length > 0) {
						...
						toastr.error(
							"no marker found for the criteria you selected"
						);
					}
				});
			} else {
		...
		function correctLatLng(marker) {
			try {
				if (
					!marker.lat ||
					marker.lat.toString().search(/[a-zA-Z]/i) > -1
				)
					marker.lat = 12.971599;
				if (
					!marker.lng ||
					marker.lng.toString().search(/[a-zA-Z]/i) > -1
				)
		...
		function mapProductsfiltered() {
			// $scope.isProductsLoading = true;
			MapService.mapProductsfiltered().then(function (markers) {
				// MapService.mapProductsfiltered($scope.page_params).then(function (resMarkers) {
		...
		// const productListDiv = jQuery('.table-responsive');
		...
		$scope.setCountry = function () {
			LocationService.getStates($scope.selectedCountry).then(function (
				states
			) {
				$scope.states = states;
			});
		...
		$scope.setStates = function () {
			LocationService.getCities($scope.selectedStates).then(function (
				cities
			) {
				$scope.cities = cities;
			});
		...
		$scope.setCities = function () {
			LocationService.getAreas($scope.selectedcitys).then(function (
				areas
			) {
				$scope.areas = areas;
			});
		...
		function selectMarker(marker) {
			...
			$scope.product.image =
				config.serverUrl + marker.properties["image"];
			...
			$scope.product.secondImpression =
				marker.properties["secondImpression"];
			$scope.product.firstImpression =
				marker.properties["firstImpression"];
			$scope.product.thirdImpression =
				marker.properties["thirdImpression"];
			$scope.product.forthImpression =
				marker.properties["forthImpression"];
		...
		$scope.processMarkers = function () {
			...
			function addUniqueMarker(markerData) {
				...
				markersOnMap.push(marker);
				$scope.Clusterer.addMarker(marker);
				// console.log(google.maps,"hello", "marker", $scope.Clusterer)
				// google.maps.addListener(marker, 'click', function (e){
				//     console.log("start")
				// })
				google.maps.event.addListener(marker, "click", function (e) {
					$window.location.href =
						"/owner/demo---landmark-ooh/location";
					selectMarker(marker);
					// setTimeout(() => {
		...
		function getShortListedProducts() {
			MapService.getshortListProduct(
				...
				shortListedProductsLength =
					response.shortlisted_products.length;
				$scope.shortListedProducts = response;
				$scope.shortListedProducts = response.shortlisted_products;
				$scope.shortListedTotal = response.shortlistedsum;
				$rootScope.$emit(
		...
		$scope.saveCampaign = function (product_id, selectedDateRanges) {
			...
			if ($scope.campaign.products) {
				CampaignService.saveUserCampaign($scope.campaign).then(
					function (response) {
						if (response.status == 1) {
							//$scope.campaignSavedSuccessfully = true;
							$timeout(function () {
		...
		$scope.selectFromTabIdSearch = function (marker) {
			try {
				...
				if (marker.id) {
					...
					$scope.product.area_name =
						refToMapMarker.properties["area_name"];
					$scope.product.panelSize =
						refToMapMarker.properties["panelSize"];
					$scope.product.Comments =
						refToMapMarker.properties["Comments"];
					...
					$scope.product.billing =
						refToMapMarker.properties["billingNo"] == "no"
							? "No"
							: "Yes";
					$scope.product.servicing =
						refToMapMarker.properties["servicingNo"] == "no"
							? "No"
							: "Yes";
					$scope.product.ageloopLength =
					...
					$scope.product.spotLength =
						refToMapMarker.properties["spotLength"];
					$scope.product.isSpotLengthExist =
						$scope.product.spotLength === "" ? false : true;
					$scope.product.unitQty =
						refToMapMarker.properties["unitQty"];
					$scope.product.mediahhi =
						refToMapMarker.properties["mediahhi"];
					$scope.product.isMediaHiExist =
						$scope.product.mediahhi === "" ? false : true;
					$scope.product.vendor = refToMapMarker.properties["vendor"];
					$scope.product.strengths =
						refToMapMarker.properties["strengths"];
					$scope.product.address =
						refToMapMarker.properties["address"];
					...
					$scope.product.installCost =
						refToMapMarker.properties["installCost"];
					...
					$scope.product.fliplength =
						refToMapMarker.properties["fliplength"];
					$scope.product.fliplength =
						refToMapMarker.properties["fliplength"];
					$scope.product.weekPeriod =
						refToMapMarker.properties["weekPeriod"];
					$scope.product.rateCard =
						refToMapMarker.properties["rateCard"];
					$scope.product.venue = refToMapMarker.properties["venue"];
					$scope.product.lighting =
						refToMapMarker.properties["lighting"];
					$scope.product.direction =
						refToMapMarker.properties["direction"];
					$scope.product.availableDates =
						refToMapMarker.properties["availableDates"];
					$scope.product.slots = refToMapMarker.properties["slots"];
					$scope.product.file_type =
						refToMapMarker.properties["file_type"];
					...
					$scope.product.placement =
						refToMapMarker.properties["placement"];
					...
					$scope.product.fix = refToMapMarker.properties["fix"];
					$scope.product.minimumdays =
						refToMapMarker.properties["minimumdays"];
					$scope.product.length = refToMapMarker.properties["length"];
					$scope.product.network =
						refToMapMarker.properties["network"];
					$scope.product.nationloc =
						refToMapMarker.properties["nationloc"];
					$scope.product.daypart =
						refToMapMarker.properties["daypart"];
					...
					var fromtime =
						refToMapMarker.properties.from_date.$date.$numberLong;
					$scope.product.fromTime = moment(
						new Date(+fromtime)
					).format("YYYY-MM-DD");
					var endTime =
						refToMapMarker.properties.to_date.$date.$numberLong;
		...
		$scope.selectFromImpressionSearch = function (marker) {
			...
			if (marker.id) {
				...
				$scope.product.fliplength =
					refToMapMarker.properties["fliplength"];
				$scope.product.looplength =
					refToMapMarker.properties["looplength"];
				$scope.product.installCost =
					refToMapMarker.properties["installCost"];
				$scope.product.staticMotion =
					refToMapMarker.properties["staticMotion"];
				...
				$scope.product.cpm = refToMapMarker.properties["cpm"];
				$scope.product.firstcpm =
					refToMapMarker.properties["firsstcpm"];
		...
		// }
		$scope.toggleFormatSelection = function (
			formatId,
			booked_from,
			booked_to
		) {
			if (_.contains($scope.selectedFormats, formatId)) {
				$scope.selectedFormats = _.reject(
					$scope.selectedFormats,
					function (v) {
						return v == formatId;
					}
		...
		$scope.updateCircle = function () {
			...
			rangeCircle.setRadius(
				Math.sqrt(($scope.circleRadius * 1000) / Math.PI)
			);
		...
		$scope.addProductToExistingCampaign = function (
			...
			CampaignService.addProductToExistingCampaign(
				productToCampaign
		...
		$scope.shareShortlistedProducts = function (shareShortlisted) {
			...
			CampaignService.shareShortListedProducts(sendObj).then(function (
				result
			) {
				if (result.status == 1) {
					toastr.success(result.message);
		...
		$scope.getProductDigitalUnavailableDates = function (
			...
			$scope.digitalSlots = [];
			MapService.getProductDigitalUnavailableDates(productId).then(
				function (blockedDatesAndSlots) {
		...
		$scope.getProductUnavailableDates = function (product, ev) {
			// if(product.type == "Bulletin"){
			if ($scope.areaObj) {
				MapService.getProductUnavailableDates(product.id).then(
					function (dateRanges) {
						$(".warn-text").text("");
						$scope.unavailalbeDateRanges = dateRanges;
		...
		$scope.FormatData = function (selectedZone) {
			$scope.loading = true;
			$scope.productmarkerslist = $scope.actualDataCopy.filter(function (
				item
			) {
				$scope.loading = false;
		...
		$scope.FilterProductlist = function (booked_from, booked_to) {
			MapService.filterProducts(booked_from, booked_to).then(function (
				result
			) {
				productList = [];
				locArr = [];
		...
		$scope.saveCampaignName = function (
			...
			if (
				$scope.totalnumDays <
				$scope.product.minimumbooking.split(" ")[0]
			) {
				toastr.error(
					"minimum you have to select " +
						$scope.product.minimumbooking
				);
				return false;
			}
			...
			CampaignService.payAndLaunch(paylaunchProduct).then(function (res) {
				if (res.status == 1) {
					...
					$scope.toggleProductDetailSidenav();
					$window.location.href =
						"/campaign-details/" + res.campaign_id + "/1";
		...
		$scope.saveDigitalCampaignName = function (
			...
			var startAndEndDates = selectedDateRanges.filter(
				(item) => item.selected
			);
			$scope.loading = true;
			startAndEndDates.forEach((item, index) => {
		...
		$scope.$watch("ranges.selectedDateRanges", function () {
			...
			for (item in $scope.ranges.selectedDateRanges) {
				...
				$scope.totalnumDays = $scope.totalnumDays + totalDays;
				$scope.totalPriceUserSelected =
					productPerDay * $scope.totalnumDays;
