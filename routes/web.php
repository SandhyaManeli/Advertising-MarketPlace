...
$app->group(['prefix' => 'api'], function () use ($app){
    ...
    $app->post('export-product-details', ['uses' => 'ProductController@exportProductDetails']);
	$app->get('exported-columns/{report_type}', ['uses' => 'ProductController@getExportedProducts']);
	$app->post('exported-columns-update', ['uses' => 'ProductController@getExportedColumnsUpdate']);
    $app->get('product-availabilty-quantity/{product_id}/{from_date}/{to_date}', ['uses' => 'ProductController@getProductAvailabilityQuantity']);

    $app->get('get-all-map-products', ['uses' => 'ProductController@getAllMapProducts']);
    $app->get('get-single-product-details', ['uses' => 'ProductController@getSingleProductDetails']);
	
	$app->get('get-chat-messages/{campaign_id}', ['uses' => 'ProductController@get_chat_messages']);
	$app->post('get-create-messages', ['uses' => 'ProductController@get_create_chat_messages']);
	$app->post('product-search-criteria', ['uses' => 'ProductController@ProductsSearchCriteriaDetails']);
	$app->get('product-search-filters-data', ['uses' => 'ProductController@GetProductSearchFiltersData']);
	$app->get('product-search-filters-data-by-id/{search_id}', ['uses' => 'ProductController@GetProductSearchFiltersDataById']);
    $app->post('products-transfer', ['uses' => 'ProductController@GetTransferProductUpdate']);  
    $app->post('get-products-by-lat-long', ['uses' => 'ProductController@GetProductsByLatLong']);  
    $app->get('get-radius', ['uses' => 'ProductController@calculateRadius']);  
	$app->get('findProductsInPolyline', ['uses' => 'ProductController@findProductsInPolyline']);  
	$app->get('pointInPolygon', ['uses' => 'ProductController@pointInPolygon']);  
	$app->get('get-assaign-rfp-products/{campaign_id}', ['uses' => 'ProductController@GetAssaignRfpToProduct']);  
	...
	$app->post('product-expiry-notify', ['uses' => 'ProductController@productExpiryNotification']);
    $app->get('get-expiry-products', ['uses' => 'ProductController@getProductExpiryNotify']);
	...
	$app->get('search-buyers/{search_term}', ['uses' => 'UserController@searchBuyers']);
	$app->post('transfer-campaign-to-buyer', ['uses' => 'UserController@transferCampaignToBuyer']);
    ...
    $app->get('user-saved-campaigns', ['uses' => 'CampaignController@getUserSavedCampaigns']);
    ...
    $app->post('update-rfp-status', ['uses' => 'CampaignController@updateRFPStatus']);
    $app->post('update-todolist-status', ['uses' => 'CampaignController@updateToDoListStatus']);
	...
	$app->post('update-campaign-product-date-quantity', ['uses' => 'CampaignController@updateCampaignProductDateQuantity']);
	$app->get('download-inerstion-order/{campaign_id}', ['uses' => 'CampaignController@downloadInsertionOrder']);
	$app->post('clone-saved-campaign', ['uses' => 'CampaignController@cloneSavedCampaign']);
	$app->post('unit-quantity-unavailable-mail', ['uses' => 'CampaignController@unitQuantityUnavailableEmail']);
	$app->post('offer-accept-reject-for-seller', ['uses' => 'CampaignController@offerAcceptRejectForSeller']);
	$app->get('offer-accept-reject-comments/{booking_id}', ['uses' => 'CampaignController@OfferAcceptRejectComments']);
    $app->post('delete-multiple-products-from-campaign', ['uses' => 'CampaignController@deleteMultipleProductFromCampaign']);
	$app->post('campaign-gross-fee-update', ['uses' => 'CampaignController@grossFeeUpdateCampaign']);
    $app->get('campaign-gross-fee-get/{campaign_id}', ['uses' => 'CampaignController@grossFeeGetCampaign']);
	$app->get('rfp-search-criteria-download/{campaign_id}', ['uses' => 'CampaignController@RFPSearchCriteriaDownload']);
	$app->post('find-for-rfp-campaign', ['uses' => 'CampaignController@findForRFPCampaign']);
	$app->post('send-add-product-request-to-seller', ['uses' => 'CampaignController@sendAddProductRequestToSellers']);
	$app->get('get-rfp-search-criteria-requests', ['uses' => 'CampaignController@getRFPSearchCriteriaRequests']);
...
//APIs For Product Adding.
 ...
 $app->get('generate-pop/{campaign_id}', ['uses' => 'CampaignController@generatePop']);
  ...
  //For Sub Seller
   ...
   // Bulk Upload
   $app->post('import', ['uses' => 'ProductsBUController@import']);
   $app->post('save-bulk-upload-images', ['uses' => 'ProductsBUController@saveBulkUploadImages']);
   $app->get('get-bulk-upload-images', ['uses' => 'ProductsBUController@getBulkUploadImages']);
   $app->get('get-all-sellers', ['uses' => 'ProductsBUController@getAllSellers']);
   $app->get('get-bu-products-by-seller', ['uses' => 'ProductsBUController@getBUProductsBySeller']);

   // Contracts & Invocies Module
   $app->get('get-contracts', ['uses' => 'CampaignController@getContracts']);
   $app->post('save-contracts', ['uses' => 'CampaignController@saveContracts']);
   $app->get('get-invoices', ['uses' => 'CampaignController@getInvoices']);
   $app->post('save-invoices', ['uses' => 'CampaignController@saveInvoices']);
   $app->post('delete-contracts', ['uses' => 'CampaignController@deleteContracts']);
   $app->post('delete-invoices', ['uses' => 'CampaignController@deleteInvoices']);
   $app->post('download-contracts', ['uses' => 'CampaignController@downloadContracts']);
   $app->post('download-invoices', ['uses' => 'CampaignController@downloadInvoices']);
});
