...
class metroCampaignClosedEvent implements ShouldBroadcast {
    ...
    public function broadcastOn() {
        if (Notification::$NOTIFICATION_CLIENT_TYPE['bbi'] == $this->to_type) {
            ...
            return ['metroCampaignClosed-superAdmin'];
        } else if (Notification::$NOTIFICATION_CLIENT_TYPE['owner'] == $this->to_type) {
            foreach ($this->to_client as $value) {

                $arr[] = 'metroCampaignClosed-' . $value;
            }
            return $arr;
        } else {
            $user_mongo = JWTAuth::parseToken()->getPayload()['userMongo'];
            //return ['metroCampaignClosed'.$user_mongo['id']];
            return ['metroCampaignClosed' . $this->to_client];
        }
    }

    public function broadcastAs() {
        return 'metroCampaignClosedEvent';
    }
